...
public class GalacticSearchA extends SequentialCommandGroup {
  /** Creates a new GalacticSearchA. */
  public GalacticSearchA(Drive drive, Tower tower, Sorter sorter, FloorIntake intake) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new LowerFloorIntake(intake),
      new ParallelDeadlineGroup(  
        new TravelPath(Paths.get("/home/lvuser/deploy/galacticSearchBeginningA.wpilib.json"), drive, false),
    ...
    // If this is true we are on the red path
      addCommands(
        new ParallelDeadlineGroup(  
          new TravelPath(Paths.get("/home/lvuser/deploy/galacticSearchRedA.wpilib.json"), drive, false),
          new RunFloorIntake(intake)
        )
      );
    }
    ...
    // If this is running we are on the blue path
      addCommands(
        new ParallelDeadlineGroup(  
          new TravelPath(Paths.get("/home/lvuser/deploy/galacticSearchBlueA.wpilib.json"), drive, false),
          new RunFloorIntake(intake)
        )
      );
    }
